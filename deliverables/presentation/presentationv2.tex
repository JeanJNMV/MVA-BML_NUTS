...
\begin{frame}{Summary \& Takeaways}
  \noindent
    \begin{tcolorbox}[keybox, title=Paper strengths]
      \small
      \begin{itemize}
        \item Resolves HMC's central practical limitation through a conceptually simple yet theoretically rigorous construction
        \item No-U-Turn criterion and recursive doubling preserve detailed balance
        \item Dual averaging for step-size adaptation $\Rightarrow$ essentially tuning-free
      \end{itemize}
    \end{tcolorbox}
    ...
    \begin{tcolorbox}[warnbox, title=Limitations]
      \small
      \begin{itemize}
        \item Dual-averaging hyperparameters justified empirically, not theoretically; robustness across different geometries is open
        \item Performance guarantees mainly asymptotic
        \item Experiments cover a limited range of posterior geometries, leaving pathological targets unexplored
      \end{itemize}
    \end{tcolorbox}
...
\begin{frame}{Our Contributions}
  \begin{tcolorbox}[keybox]
    ...
    \begin{itemize}
      ...
      \item \textbf{Stress tests:} adaptive trajectory length is critical on complex posteriors --- gains most dramatic on Funnel \& BNN; advantage narrows on dense GP; ESS/grad metric conceals NUTS's algorithmic overhead
